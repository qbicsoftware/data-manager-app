 ...
 * <p>
 * The Exchange class is a simple implementation of the {@link MessageBusInterface} and can be used to broadcast
 * messages out of the data manager and user management context.
 * <p>
 * Note: use this class for development purposes only and replace it with an implementation that utilizes
 * a production grade messaging middleware, such as for example RabbitMQ or Apache Kafka.
 *
 * @since <version tag>
 */
public class Exchange implements MessageBusInterface {
    ...
    /**
     * Submits a message to the exchange. The topic is taken from the {@link MessageParameters#messageType} parameter,
     * and all subscriber to this topic are informed.
     *
     * @param message           the message to publish via the exchange instance
     * @param messageParameters some message parameters, such as the type (aka topic), the occuredOn timepoint and
     ...
     *
     * @param subscriber the subscriber callback reference. A subscriber can only subscribe once to a topic. Multiple
     *                   calls will not overwrite the subscriber.
     * @param topic      the topic to subscribe to
     */
    @Override
    synchronized public void subscribe(MessageSubscriber subscriber, String topic) {

...
-- Per ADR-0002 S2: per-user, per-instance personal access tokens for external
-- data providers. The master AES-256 key lives in the PKCS12 vault (deploy
-- time); each token is individually AES-GCM-encrypted (nonce + ciphertext +
-- tag) and stored as a VARBINARY blob. Any HA node can decrypt (same key,
-- shared DB).
--
-- The table is source-agnostic: `source_type` + `instance_id` together
-- identify the provider and instance. Adding a second provider requires no
-- schema change — just insert rows with a different `source_type`.
-- ===========================================================================
